...
Take the UK population of 70 million. We have a sample of this, and in our sample some observations are male and some female. How can we use what we have, a sample, to estimate the probability that an individual is male?[^05-introtomaximumlikelihood-1]

[^05-introtomaximumlikelihood-1]: In the future, we may want to know the probability of turning out to vote, or of going to war, or voting "yes" on a particular policy question, etc.
...
-   Our issue (or at least one of our issues) is that products are tough to differentiate. A chain rule disaster. Instead, we use a trick of taking the log of the likelihood: log $\prod ()$.[^05-introtomaximumlikelihood-2] Benefit: it turns it into a sum, much easier. $\log ab = \log a + \log b$. So we will actually differentiate the $\log$ of the likelihood. Yes, this is why we had logs as part of Section 3.

[^05-introtomaximumlikelihood-2]: Why does this work? It has to do with the shape of the log (always increasing). Details are beyond the scope.
...
\end{align*}
```
Note: we assume our observations are iid (independently and identically distributed (For [definition](http://www.statisticshowto.com/iid-statistics/%7D).) This assumption can be relaxed.
...
{{< video src="https://www.youtube.com/watch?v=5QFj6_D-aAw" >}}

Take the derivative wrt $\theta$. Note: we have to take two derivatives- one for $\mu$ ($\beta$) and one for $\sigma^2$. For this example we will focus only on the derivative wrt to $\beta$, as that it what gets us the coefficient estimates.[^05-introtomaximumlikelihood-3]

[^05-introtomaximumlikelihood-3]: Essentially, you need to take derivatives with respect to each of the parameters. Some models we use will have only one parameter, which is easier.
