...
[^10-countdata-1]: Count data will always prevent the errors from being normality distributed, which can be problematic for estimates in small samples. In large samples, the uncertainty estimates will still approximate the correct values.
...
avg.exp.counts.me <- avg_predictions(fit,by="divided",
                                     newdata= datagridcf(divided=c(0,1)))
avg.exp.counts.me
```
...
library(marginaleffects)
avg.diff.me <- avg_comparisons(fit, variables=list(divided=c(0,1)))
avg.diff.me
...
![](images/science1.png){width="70%"} Recall that for our primary count models, the regression equation is of the form:
...
count.byage.me <- avg_predictions(model1.qp, by="age", newdata=datagridcf(age = levels(fake$age)))
count.byage.me
