...
-   Oftentimes count data are heavily right-skewed and very sparse.
    ...
    -   Particularly in small samples, OLS can struggle with heavily skewed data because the error variance is likely not going to be homogenous, the distribution of errors is not going to be normal[^10-countdata-1], and the linearity assumption could very well be suspect (violations of the usual assumptions).
...
[^10-countdata-1]: Count data will always prevent the errors from being normally distributed, which can be problematic for estimates in small samples. In large samples, the uncertainty estimates will still approximate the correct values.
...
<summary>To filter the histogram by party like Fig. 1 in the paper, expand.</summary>
...
This [blog](https://econometricsense.blogspot.com/2017/03/count-models-with-offsets.html) post has a set of questions to ask if you should include an offset."(1) What is the relevant interval in time or space upon which our counts are based? (2) Is this interval different across our observations of counts?"
...
{{< video src=“https://www.youtube.com/watch?v=sU-_rcLVTMw” >}}
